...
class Nettack(BaseAttack):
    ...
    def __init__(self, model, nnodes=None, attack_structure=True, attack_features=False, device='cpu'):
        ...
        super(Nettack, self).__init__(model, nnodes, attack_structure=attack_structure, attack_features=attack_features,
                                      device=device)
    ...
    def attack(self, features, adj, target_node, num_users, n_perturbations, direct=True, n_influencers=0, ll_cutoff=0.004,
               verbose=True, **kwargs):
        ...
        if type(adj) is torch.Tensor:
            ...
            if self.attack_features and features is not None:
        ...
        if len(self.influencer_nodes) == 0:
            if not direct:
                ...
                self.potential_edges = np.column_stack(
                    (np.tile(target_node, self.nnodes - 1), np.setdiff1d(np.arange(self.nnodes), target_node)))
        ...
        for _ in range(n_perturbations):
            ...
            self.modified_adj[tuple(filtered_edges_final)] = self.modified_adj[tuple(filtered_edges_final[::-1])] =\
                1 - self.modified_adj[tuple(filtered_edges_final)]
            self.adj_norm = utils.normalize_adj(self.modified_adj)
